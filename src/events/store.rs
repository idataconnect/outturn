...
pub async fn since(
    ...
    let rows = match session_id {
        Some(sid) => {
            sqlx::query(
                "select id, workspace_id, session_id, kind, payload from events \
...
pub async fn wait_for(
    ...
    let existing = since(pool, workspace_id, session_id, after, limit).await?;
        ...
        tokio::select! {
            ...
            received = rx.recv() => match received {
                Ok(hint) => {
                    ...
                    let events = since(pool, workspace_id, session_id, after, limit).await?;
                    if !events.is_empty() {
                        return Ok(events);
                    }
                }
