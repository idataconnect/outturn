...
`sessions:read`, `storage:tenant:write`. They are an enum in
...
**Roles** bundle authorities under a name a person recognises. For tenants,
roles are data: rows in `roles` and `role_authorities`, owned by the tenant,
created and edited by whoever holds `roles:manage` there. Every tenant starts
...
else may define what they mean, and the role store refuses to create a tenant
...
means whatever the tenant's rows say at the moment of the request. Editing a
...
- `RoleStore::authorities_for` for tenant roles, from the rows. Cached per
  tenant in each API pod. A write to a tenant's roles announces itself on the
  `outturn_roles` Postgres channel and every pod drops that tenant's entry, so
  there is no TTL to guess at.

## Guards, because roles are editable now

A role may bundle only authorities that are *tenant-assignable*. Platform-wide
authorities -- `tenants:*`, `work:take` -- are refused, whoever asks. This is
the successor to the check constraint that used to pin role names: a tenant
that could grant itself `tenants:create` could make tenants.
...
Which roles a person holds in a tenant is answered today by
`user_tenant_roles`, and by nothing else. It is deliberately not the same
...
through a tenant-scoped table. Authorities would still come only from the
tenant's own rows; an external system could say "this person is in group X",
...
| Scope   | Read                   | Write                   |
|---------|------------------------|-------------------------|
| Tenant  | `storage:tenant:read`  | `storage:tenant:write`  |
...
roles, admin holds all of these, operator reads tenant files and reads and
