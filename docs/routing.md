...
An operator runs outturn. Workspaces are the operator's customers, and workspaces
...
credential. A skill will name a class the same way. The workspace and the
...
Built: `traffic_routes` with a null workspace meaning the operator's default and a
workspace's rows replacing it; attempts tried in priority order, skipping any
...
*operator-held* (an env var, as now) or *workspace-held* (a key the workspace
...
this widens nothing; the API proxies the workspace's submission and learns only
that a key is on file and its last few characters.

A workspace's list can then mix freely: Bedrock with their AWS credentials, then
...
Designed. A workspace with their own key may opt into "during provider outages,
fall back to the operator's model; additional charges apply." It is just the
operator's rows appended after the workspace's, and one fact on the bill: those
calls were paid with the operator's credential.

- Off by default, per workspace.
- The operator decides whether to offer it at all. A hosting-only workspace has
  no usage relationship to charge against and never sees the checkbox.
- The breaker decides when it engages, not the workspace. The workspace's rows are
  always tried first, so nobody routes to the operator's model by preference.
- Fallback is per model call. A turn that fails mid-stream on the workspace's
...
Designed: split it, because two different things trip it once workspaces hold
keys.

- **Endpoint health** stays cross-workspace. Connection refused, timeouts, 5xx.
...
- **Credential health** is per credential. 401 and 403 mean this key is bad;
  ...
  longer one for 401, never propagated to the endpoint breaker. A workspace's
  expired key must not take a provider down for the other workspaces.
...
Today a 429 does not count at all, so a workspace hammering their own key fails
...
owner and fallback kind on the rows, workspace-held keys in the gateway, the
